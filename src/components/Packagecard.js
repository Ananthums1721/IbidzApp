...
import { SafeAreaView, StyleSheet, View, Text, Dimensions, TouchableOpacity, ImageBackground } from 'react-native';
    ...
    const getRazorpayInfo = async () => {
      try {
        ...
        if (res != '') {
          ...
          Toast.show('Sorry! Paymet is unable to initiate at this time. Please contact our support.')
        }
      } catch (error) {
        showLoader(false)
      }
    ...
    return(
      ...
      >
        <View style={styles.priceCon}>
          <Text style={[styles.fontText1, { color: colours.blue }]}>₹{packageAmount}<Text style={styles.fontText6}> +GST</Text></Text>
        </View>
        <View style={styles.nameCon}>
            <Text style={styles.fontText3}>{packageName}  <Text style={[styles.fontText4,{color: colours.primaryWhite}]}>({packageAuction} BID tickets)</Text></Text>
        </View>
        <View style={styles.detailsCon}>
          <View style={styles.detailsInCon}>
            <Text style={styles.fontText4}>The {packageName} plan includes {packageAuction} auctions up to {packageValidity} Day(s)</Text>
          </View>
          <View style={styles.detailsInCon}>
            <Text style={styles.fontText4}>{packageAuction} Auctions</Text>
          </View>
          <View style={[styles.detailsInCon,{borderBottomWidth:0}]}>
            <Text style={[styles.fontText4]}>{packageValidity} day(s) Pack validity</Text>
          </View>
        </View>

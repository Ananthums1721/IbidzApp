...
import { forgotPassword, resentOTP, verifyOTP } from '../api';
...
const VendorForgotPassword = ({ navigation, route }) => {
        ...
        if (phone != '' || email != '') {
            try {
                ...
                let res = await forgotPassword({
                    ...
                    // username: `+91-${phone}`
                    username: data.split("+91").join("+91-")
                })
                if (res != '') {
                    Toast.show({
                        type: 'success',
                    ...
                    navigation.navigate('VendorResetPassword',{ id: res[0].sellerId})
                }
                showLoader(false);
            } catch (e) {
                Toast.show({
    ...
    return (
        ...
        <Header backarrow title={'Forgot Password'} navigation={navigation} />
